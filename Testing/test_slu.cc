...
main(int argc, char** argv)
{
  ...
  slu_init_thermal_data (&stkd, &tdata, TL_CCS_MATRIX, 300.00, delta_time) ;
  ...
  slu_fill_thermal_data (&stkd, &tdata) ;
  ...
  {
    slu_print_temps (&tdata, &stkd, time) ;
#else
    slu_print_temps (&tdata, &stkd, 0.0) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    slu_print_temps (&tdata, &stkd, time) ;
#else
    slu_print_temps (&tdata, &stkd, 0.10) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    slu_print_temps (&tdata, &stkd, time) ;
#else
    slu_print_temps (&tdata, &stkd, 0.20) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    slu_print_temps (&tdata, &stkd, time) ;
#else
    slu_print_temps (&tdata, &stkd, 0.30) ;
#endif
...
#else
    slu_print_temps (&tdata, &stkd, 0.40) ;
#endif

exit :

  slu_free_thermal_data (&tdata) ;
