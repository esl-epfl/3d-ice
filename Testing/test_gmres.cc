...
void
gmres_print_temps (struct GMRESThermalData *tdata, struct StackDescription *stkd, double time)
{
  int column ;
...
main(int argc, char** argv)
{
  ...
  gmres_init_thermal_data (&stkd, &tdata, 300.00, delta_time) ;
  ...
  gmres_fill_thermal_data (&stkd, &tdata) ;
  ...
  {
    gmres_print_temps (&tdata, &stkd, time) ;
#else
    gmres_print_temps (&tdata, &stkd, 0.0) ;
#endif

    tolerance = TOLERANCE ;
    max_iter = MAX_ITER ;
    result = gmres_solve_system (&tdata, sim_time, &tolerance, &max_iter, 100 ) ;
    if (result != 0)
    {
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    gmres_print_temps (&tdata, &stkd, time) ;
#else
    gmres_print_temps (&tdata, &stkd, 0.10) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    gmres_print_temps (&tdata, &stkd, time) ;
#else
    gmres_print_temps (&tdata, &stkd, 0.20) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    gmres_print_temps (&tdata, &stkd, time) ;
#else
    gmres_print_temps (&tdata, &stkd, 0.30) ;
#endif
...
#else
    gmres_print_temps (&tdata, &stkd, 0.40) ;
#endif

exit :

  gmres_free_thermal_data (&tdata) ;
