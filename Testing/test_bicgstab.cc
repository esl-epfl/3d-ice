...
void
bicgstab_print_temps (struct BICGStabThermalData *tdata, struct StackDescription *stkd, double time)
{
  int column ;
...
main(int argc, char** argv)
{
  ...
  bicgstab_init_thermal_data (&stkd, &tdata, 300.00, delta_time) ;
  ...
  bicgstab_fill_thermal_data (&stkd, &tdata) ;
  ...
  {
    bicgstab_print_temps (&tdata, &stkd, time) ;
#else
    bicgstab_print_temps (&tdata, &stkd, 0.0) ;
#endif
    ...
    result = bicgstab_diag_pre_solve_system
             (&tdata, sim_time, &tolerance, &max_iter ) ;
#endif
#ifdef TL_ILU_PRECONDITIONER
    result = bicgstab_ilu_pre_solve_system
             (&tdata, sim_time, &tolerance, &max_iter ) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    bicgstab_print_temps (&tdata, &stkd, time) ;
#else
    bicgstab_print_temps (&tdata, &stkd, 0.10) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    bicgstab_print_temps (&tdata, &stkd, time) ;
#else
    bicgstab_print_temps (&tdata, &stkd, 0.20) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    bicgstab_print_temps (&tdata, &stkd, time) ;
#else
    bicgstab_print_temps (&tdata, &stkd, 0.30) ;
#endif
...
#else
    bicgstab_print_temps (&tdata, &stkd, 0.40) ;
#endif
...
exit :
  ...
  bicgstab_free_thermal_data (&tdata) ;
