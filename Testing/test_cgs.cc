...
void
cgs_print_temps (struct CGSThermalData *tdata, struct StackDescription *stkd, double time)
{
  int column ;
...
main(int argc, char** argv)
{
  ...
  cgs_init_thermal_data (&stkd, &tdata, 300.00, delta_time) ;
  ...
  cgs_fill_thermal_data (&stkd, &tdata) ;
  ...
  {
    cgs_print_temps (&tdata, &stkd, time) ;
#else
    cgs_print_temps (&tdata, &stkd, 0.0) ;
#endif
    ...
    result = cgs_diag_pre_solve_system
             (&tdata, sim_time, &tolerance, &max_iter ) ;
#endif
#ifdef TL_ILU_PRECONDITIONER
    result = cgs_ilu_pre_solve_system
             (&tdata, sim_time, &tolerance, &max_iter ) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    cgs_print_temps (&tdata, &stkd, time) ;
#else
    cgs_print_temps (&tdata, &stkd, 0.10) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    cgs_print_temps (&tdata, &stkd, time) ;
#else
    cgs_print_temps (&tdata, &stkd, 0.20) ;
#endif
  ...
  for (time = 0.00 ; time < 0.10 ; time += delta_time)
  {
    cgs_print_temps (&tdata, &stkd, time) ;
#else
    cgs_print_temps (&tdata, &stkd, 0.30) ;
#endif
...
#else
    cgs_print_temps (&tdata, &stkd, 0.40) ;
#endif
